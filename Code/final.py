...
model = load_model('cnn_model_keras2_final.h5')
...
def get_image_size():
    ...
    if img is None:
        raise FileNotFoundError("Sample gesture image not found in gestures/1/.")
    return img.shape
...
def keras_process_image(img):
    img = cv2.resize(img, (image_x, image_y))
    img = np.array(img, dtype=np.float32)
    img = np.reshape(img, (1, image_y, image_x, 1))  # correct order
    return img
...
def calculator_mode(cam):
    ...
    flag = {"first":False,"operator":False,"second":False,"clear":False}
    count_same_frames = 0
    first, operator, second, calc_text, pred_text, info = "", "", "", "", "", "Enter first number"
    Thread(target=say_text,args=(info,)).start()
    count_clear_frames = 0
    ...
    while True:
        ...
        if contours:
            ...
            if cv2.contourArea(contour) > 10000:
                ...
                elif pred_text.isnumeric() and count_same_frames>15:
                    if not flag['first']:
                        first+=pred_text
                        calc_text+=pred_text
                        info="Enter operator"
                        flag['first']=True
                        Thread(target=say_text,args=(pred_text,)).start()
                    elif not flag['operator']:
                        operator=get_operator(pred_text)
                        calc_text+=operator
                        flag['operator']=True
                        info="Enter second number"
                        Thread(target=say_text,args=(info,)).start()
                    elif not flag['second']:
                        second+=pred_text
                        calc_text+=pred_text
                        flag['second']=True
                        Thread(target=say_text,args=(pred_text,)).start()
    ...
    while True:
        ...
        if contours:
            ...
            if cv2.contourArea(contour)>10000:
                ...
                if count_same_frame>20:
                    if len(text)==1: Thread(target=say_text,args=(text,)).start()
                    word+=text
                    count_same_frame=0
